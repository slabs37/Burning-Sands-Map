Shader "Hidden/Locked/.poiyomi/Poiyomi Toon/3de8ed2d98f24814fb5f113cdd9e81a6"
{
	Properties
	{
 ...
 #define OPTIMIZER_ENABLED 
			...
			{
				float3 parallax = 0;
				for (int j = (2.0 /*_ParallaxInternalIterations*/) - 1; j >= 0; j--)
				{
					float ratio = saturate((float)j / max((2.0 /*_ParallaxInternalIterations*/) - 1, 1));
					float2 parallaxOffset = _Time.y * (float4(0,0,1,1) + ratio * float4(0,0,1,1));
					float fade = lerp((1.0 /*_ParallaxInternalMinFade*/), (0.1 /*_ParallaxInternalMaxFade*/), ratio);
					fade = pow(fade, 2.2);
					#if defined(PROP_PARALLAXINTERNALMAP) || !defined(OPTIMIZER_ENABLED)
					float4 parallaxColor = UNITY_SAMPLE_TEX2D_SAMPLER(_ParallaxInternalMap, _MainTex, TRANSFORM_TEX(poiMesh.uv[0], _ParallaxInternalMap) + (lerp((0.0 /*_ParallaxInternalMinDepth*/), (0.01 /*_ParallaxInternalMaxDepth*/), ratio)) * - (poiCam.tangentViewDir.xy / (poiCam.tangentViewDir.z)) + parallaxOffset);
					#else
					float4 parallaxColor = 0;
					#endif
					float3 minColor = poiThemeColor(poiMods, float4(0.6698113,0.6698113,0.6698113,1).rgb, (0.0 /*_ParallaxInternalMinColorThemeIndex*/));
